 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export rdxk_data=''   ,多账号用 换行 或 @ 分割
 * 抓域名 下请求中 xxxxxx 的值填入变量
 */
 ...
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.rdxk_data : $.getdata('rdxk_data')) || '';  
 let msg, ck;       //默认不动 声明 变量
 ...
 async function tips(ckArr) {                            //函数tips  顾名思义就是提示
	 ...
	 DoubleLog(`\n========== 共找到 ${ckArr.length} 个账号 ==========`);     //这里双平台输出 几个账号
	 debugLog(`【debug】 这是你的账号数组:\n ${ckArr}`);                 //输出所有 demo_data
 }
 
 
 !(async () => {                  
	 let ckArr = await checkEnv(ckStr, "rdxk_data");             //让ckArr先去checkEnv检测变量函数那里检测一下 然后再去执行tips函数,如果变量不存在或者报错,则不执行tips
	 console.log("开始获取公告")
 ...
 async function start() {           //start函数
 
	 console.log("\n开始 签到");         //控制台输出 
	 await signin();            //执行下面的任务 函数
	 await $.wait(4 * 1000);     //等待4s
  ...
  */
 async function signin() {      //signin就是签到的意思 自己改
	 try {
		 let url = {
			 url: `${hostname}/api/usersign/sign `,     //刚才提到了  hostname
			 headers: {
				 "Host": host,         //刚才提到的host
				 "user-agent": "xxx",      //UA  
				'Referer': 'https://servicewechat.com/wx377b0efa3ce70803/33/page-frame.html',
				"Authori-zation": 'Bearer'+ck[0]
			 },

		 };
		 let result = await httpGet(url, `签到`);          //让返回值等于响应后的数据     这里是httpGet  如果你是Post  记得改一下改成httpPost
 
		 console.log(result);        //输出返回
		 if (result?.code == 200) {                                   //如果返回值中的code等于0 则
			 DoubleLog(`签到成功:${result.msg} 🎉 `);        //输出 json结构树  这个没办法演示 自己感觉吧  
			 await wait(3);
...
//----------------------------------获取公告
  ...
  async function ScriptNotice() {      
	 try {
		 let url = {
			 url: githubproxy + `https://raw.githubusercontent.com/3488790026/ymzj/tongzhi/gg.josn`,     
		 };
