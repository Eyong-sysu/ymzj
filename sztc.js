 ...
 *
 * ========= 青龙--配置文件 =========
 * 变量格式: export sztc='',多账号用 换行 或 @ 分割
 * 抓域名下请求中tcyjf.szticai.com的token值填入变量
 */
 ...
 const debug = 0	
 //---------------------------------------------------------------------------------------------------------
 let ckStr = ($.isNode() ? process.env.sztc : $.getdata('sztc')) || '';
 let msg, ck;       
 let ck_status = true;
 let host = 'tcyjf.szticai.com';
 ...
 //---------------------------------------------------------------------------------------------------------
 let Change = '1000积分兑换1彩豆10个彩豆兑换一个月会员' 
 let thank = `\n感谢 群友 的投稿\n`   
 ...
 !(async () => {                  
	 let ckArr = await checkEnv(ckStr, "sztc");             
	 await tips(ckArr);         
		 let num = index + 1;
		 DoubleLog(`\n-------- 开始【第 ${num} 个账号】--------`);
 ...
 /**
  * 签到    httpPost        //参考上面的即可  这个函数也可以复制 改下相应的就可以
  */
   async function signIn() {
	try {
		let url = {
			url: `${hostname}/api/user/sign`,
			headers: {
				"Host": host,
				"User-Agent": "xxx",
				'Content-type': 'application/json',
                'Referer': 'https://servicewechat.com/wxd8c4308770012e67/49/page-frame.html',
                 "token": ck[0]
			}
			
		};
		let result = await httpGet(url, `查询签到`);
...
function Version_Check(name) {
	return new Promise((resolve) => {
		let url = {
			url: githubproxy + `https://raw.githubusercontent.com/3488790026/ymzj/main/sztc.js`,
		}
		$.get(url, async (err, resp, data) => {
			try {
				VersionCheck = resp.body.match(/VersionCheck = "([\d\.]+)"/)
			} catch (e) {
				$.logErr(e, resp);
			} finally {
	 ...
	 return new Promise((resolve) => {
		 ...
		 $.get(url, async (err, resp, data) => {
			 try {
				 ...
				 // console.log(data);
				 console.log(`网抑云时间: ${data.cacheAt}  by--${origin. author}`)
