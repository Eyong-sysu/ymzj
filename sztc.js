 ...
 * 项目类型：小程序
 * 项目名称：深圳体彩
 * 项目抓包：抓tcyjf.szticai.com下的token填入变量
 */
...
let UserCookie = ($.isNode() ? process.env.sztc : $.getdata('sztc')) || '';
let UserCookieArr = [];
let data = '';
let msg =``;
let isSign = false;
        ...
        $.get(url, async (error, response, data) => {
            try {
                ...
                if (result.status == 200) {
                    ...
                    //console.log(txje)
                    DoubleLog(`\n 会员查询: ✅ ，${result.userinfo.score} 当前拥有：${result. userinfo.score} 积分`)
                } else {
                    DoubleLog(`\n 会员查询: ❌ ，原因是：${result.msg}`)
                }
            } catch (e) {
...
function getVersion(timeout = 3 * 1000) {
    return new Promise((resolve) => {
        let url = {
            url: `https://ghproxy.com/https://raw.githubusercontent.com/3488790026/ymzj/main/sztc.js `,
        }
